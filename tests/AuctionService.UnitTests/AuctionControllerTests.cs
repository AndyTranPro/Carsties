...
public class AuctionControllerTests
{
    ...
    public async Task UpdateAuction_WithUpdateAuctionDto_ReturnsOkResponse()
    {
        ...
        auction.Seller = "test";
        var updateAuction = _fixture.Build<UpdateAuctionDto>().Create();
    ...
    {
        // Arrange
        var auction = _fixture.Build<Auction>().With(x => x.Seller, "seller").Without(a => a.Item).Create();
        auction.Item = _fixture.Build<Item>().Without(x => x.Auction).Create();
        var updateAuction = _fixture.Build<UpdateAuctionDto>().With(a => a.Make, auction.Item.Make).Create();
        _auctionRepo.Setup(repo => repo.GetAuctionEntityById(It.IsAny<Guid>())).ReturnsAsync(auction);
